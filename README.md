...
- `server_mtu` / `client_mtu` - Interface MTU (optional, see below)
...
Set a custom MTU to avoid fragmentation issues when Path MTU Discovery (PMTUD) is broken:

```yaml
server_mtu: 1320  # Server's wg0 interface
client_mtu: 1320  # Included in generated client configs
```

- `server_mtu` - Applied to the server's WireGuard interface
- `client_mtu` - Included in client configs (`.conf` files generated by `wgm add`)
...
**When to use:**
- TLS/HTTPS connections hang through the VPN
- SSH works but large file transfers stall
- Hub-and-spoke topology where peer-to-peer traffic traverses the server (double encapsulation)
